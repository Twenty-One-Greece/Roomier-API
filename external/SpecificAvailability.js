...
router.post('/specific-availability/', (req, res) => {
    ...
    data.roomID, data.start_date, data.end_date, data.start_date,
    data.end_date, data.start_date, data.end_date, data.hotelID, data.roomID]
...
const formatResult = (rows, res, data) => {
    ...
    const specialDates = rows[6]
    let mealplanPrice = null

    if (specialDates.length >= 1) return res.send({ status: 'No rooms found.' })
    ...
    result.cancelPolicies = formatPolicies(policies, rates[0], data, result.fullPrice.price)
...
const formatPolicies = function(policies, rate, data, price) {
    ...
    policies.forEach((policy) => {
        ...
        formatedPolicies.push({
            ...
            amount: calcAmount(policy.chargeType, policy.value, rate, data, price)
        })
    })
...
const calcAmount = (chargeType, value, rate, data, price) => {
    const start_date = moment(data.start_date)
    const end_date = moment(data.end_date)
    const dayDiff = parseInt(end_date.diff(start_date, 'days'))

    if (chargeType && rate) return parseInt((price) * (value / 100))
