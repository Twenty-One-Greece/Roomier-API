...
handleResult = (res, rows, dayDiff, data) => {
  ...
  rows[0].forEach( (property) => {
    result = {
      ...
      rooms: getRooms(rows[1], rows[2], property.id, dayDiff, data, specialdates, childPolicies)
    }
...
getRooms = (rooms, rates, propID, dayDiff, data, specialdates, childPolicies) => {
  var roomsResult = []
  ...
  rooms.forEach( (room, i) => {
    ...
    if (propID === room.propertyID) {
      ...
      specialdates.forEach( (date) => {
        ...
        const dateTest = moment(date.date).isBetween(data.start_date, data.end_date)
        const toDateTest = moment(date.toDate).isBetween(data.start_date, data.end_date)
